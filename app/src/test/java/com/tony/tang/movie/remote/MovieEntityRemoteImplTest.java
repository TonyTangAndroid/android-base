...
import com.tony.tang.movie.TestHelper;
...
public class MovieEntityRemoteImplTest {
    ...
    public void getCorrectResult() throws Exception {
        ...
        String json = TestHelper.json("fargo_remote.json", this);
    ...
    public void handleInvalidApiKeyCorrectly() throws Exception {
        String json = TestHelper.json("401.json", this);
        ...
        Truth.assertThat(testObserver.errors().get(0)).isInstanceOf(DomainException.class);
        Truth.assertThat(((DomainException) testObserver.errors().get(0)).getCode()).isEqualTo(7);
    ...
    public void handleEmptyQueryCorrectly() throws Exception {
        String json = TestHelper.json("422.json", this);
        this.mockWebServer.enqueue(new MockResponse().setResponseCode(422).setBody(json));
        this.movieRemote.list("").subscribe(testObserver);
        testObserver.awaitTerminalEvent();
        Truth.assertThat(testObserver.errors().size()).isEqualTo(1);
        Truth.assertThat(testObserver.errors().get(0)).isInstanceOf(DomainException.class);
        Truth.assertThat(((DomainException) testObserver.errors().get(0)).getCode()).isEqualTo(422);
    ...
    public void handle500() throws IOException {
        String json = TestHelper.json("500.txt", this);
        this.mockWebServer.enqueue(new MockResponse().setResponseCode(500).setBody(json));
        this.movieRemote.list("").subscribe(testObserver);
        testObserver.awaitTerminalEvent();
        Truth.assertThat(testObserver.errors().size()).isEqualTo(1);
        Truth.assertThat(testObserver.errors().get(0)).isInstanceOf(DomainException.class);
        Truth.assertThat(((DomainException) testObserver.errors().get(0)).getCode()).isEqualTo(500);
    ...
    public void handleRandomError() throws IOException {
        String json = TestHelper.json("403.txt", this);
        this.mockWebServer.enqueue(new MockResponse().setResponseCode(403).setBody(json));
        this.movieRemote.list("").subscribe(testObserver);
        testObserver.awaitTerminalEvent();
        Truth.assertThat(testObserver.errors().size()).isEqualTo(1);
        Truth.assertThat(testObserver.errors().get(0)).isInstanceOf(DomainException.class);
        Truth.assertThat(((DomainException) testObserver.errors().get(0)).getCode()).isEqualTo(403);
